...
export default function ProductGrid({ initialProducts = [] }: { initialProducts?: Product[] }) {
  const { products: storeProducts, setProducts, loadProducts, subscribeProducts } = useAdmin();
  ...
  const displayProducts = storeProducts.length > 0 ? storeProducts : initialProducts;

  const filtered = useMemo(() => activeCategory === 'Todos' 
    ? displayProducts 
    : displayProducts.filter(p => p.category === activeCategory), [activeCategory, displayProducts]);
  ...
  return (
    <section id="produtos" className="relative py-24 px-4 sm:px-6 lg:px-8">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(180deg, #00BFFF55, transparent)' }}
      />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            <Zap size={14} className="text-ice-blue fill-ice-blue" stroke="none" />
            <span
              className="text-[11px] font-mono tracking-[0.4em] uppercase text-ice-blue"
            >
              Coleção Atual
            </span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <div className="relative">
              <button
                ...
                className="btn-outline-ice flex items-center gap-2 px-4 py-2.5 font-body font-semibold text-[0.8rem] tracking-[0.1em] rounded-sm"
              >
                <SlidersHorizontal size={12} className="text-ice-blue" />
          ...
          {categories.map((cat) => (
            <motion.button
              ...
              className={`px-4 py-2 text-[11px] font-mono tracking-[0.15em] uppercase rounded-sm transition-all duration-300 ${
                activeCategory === cat
                  ? 'bg-ice-gradient text-black font-bold shadow-ice-md'
