...
export default function ProdutosPage() {
  return (
    <main className="relative bg-white dark:bg-black min-h-screen">
      ...
      <div className="pt-24"> {/* Add padding for the fixed navbar */}
        <TickerBanner />
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-4 gap-8">
            {siteConfig.features.map((f) => (
              <div key={f.label} className="text-center space-y-2">
                ...
                <h4 className="text-base font-display text-black/80 dark:text-white/80 tracking-[0.08em]">
                  {f.label}
                </h4>
                <p className="text-xs font-body text-black/50 dark:text-white/30">
                  {f.sub}
                </p>
              </div>
            ))}
